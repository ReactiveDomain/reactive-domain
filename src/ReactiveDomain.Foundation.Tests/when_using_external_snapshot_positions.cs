...
/// <summary>
/// A relay-fed read model does not read every stream it depends on: something else — a relay, a
/// shared category reader — hands it events from a source it never listens to. Its snapshot has
/// to carry that source's position anyway, or the feeding side has nowhere to resume from. These
/// tests pin that the external positions round-trip, that a state without them is unchanged, and
/// that an external entry never turns into a listener (which would double-deliver the stream).
/// </summary>
...
public sealed class when_using_external_snapshot_positions : IClassFixture<StreamStoreConnectionFixture> {
	...
	private ReadModelState StateWith(List<Tuple<string, long>>? externalCheckpoints) =>
		new(
			nameof(RelayFedReadModel),
			[new Tuple<string, long>(_ownStream, 9)],
			new RelayFedReadModel.MyState { Count = 10, Sum = 20 },
			externalCheckpoints);

	[Fact]
	public void external_positions_round_trip_untouched() {
		var snapshot = StateWith([new Tuple<string, long>(_relayedStream, 4)]);
		...
		Assert.Equal(_relayedStream, external.Item1);
		Assert.Equal(4, external.Item2);
		// The model's own checkpoints are unaffected by the external entry.
		Assert.NotNull(roundTripped.Checkpoints);
		Assert.Equal(_ownStream, Assert.Single(roundTripped.Checkpoints).Item1);
	}
	...
	public void an_unrecorded_external_stream_reports_no_position() {
		using var rm = new RelayFedReadModel(
			_configuredConnection, _relayedStream, StateWith([new Tuple<string, long>(_relayedStream, 4)]));

		Assert.False(rm.HasExternalCheckpoint("no.such.stream", out var position));
		Assert.Equal(-1, position);
	}

	[Fact]
	public void external_positions_never_start_a_listener() {
		var snapshot = StateWith([new Tuple<string, long>(_relayedStream, 1)]);
		...
		Assert.Equal(_ownStream, Assert.Single(rm.GetCheckpoint()).Item1);
	}
	...
	public void a_relayed_position_is_carried_into_the_next_snapshot() {
		...
		Assert.Equal(3, Assert.Single(saved.ExternalCheckpoints).Item2);
	...
	public sealed class RelayFedReadModel :
		...
		public void Relay(ExternalPositionTestEvent @event, long position) {
			DirectApply(@event);
			SetExternalCheckpoint(_relayedStream, position);
		}

		/// <summary>Exposes the protected accessor so the tests can assert on it.</summary>
		public bool HasExternalCheckpoint(string streamName, out long position) =>
			TryGetExternalCheckpoint(streamName, out position);
		...
		protected override void ApplyState(ReadModelState snapshot) {
			...
			Count = state.Count;
			Sum = state.Sum;
			if (TryGetExternalCheckpoint(_relayedStream, out var position)) {
				PositionSeenWhileRestoring = position;
			}
		}
