            ...
            while (!_stop)
            {
                ...
                try
                {
                    ...
                    else       // epochMs >= msg.TimeoutMs
                    {
                        ...
                        if (overdueMs > 100)
                        {
                            Log.Warn("Message should have timed out at " + msg.TimeoutMs + ", but it's now " + epochMs + " which is " + overdueMs + " late");
                        }
                        msg.SendTimeout();
                    }
                }
                ...
                {
                    Log.Error(ex,$"Error while processing message {msg} in queued handler TimeoutService.");
                }
            }
            _stopped.Set();
