...
namespace ReactiveDomain.Policy.Tests
{
        ...
        IHandle<OtherMsg>
    {
        ...
        private readonly string _applicationName = "ReactiveDomain.Policy";
        ...
        [Fact]
        public void can_get_policy_dto() {
            var file = Path.GetTempFileName();
            var clientName = "TestClient";
            var policyDto = _policyMap.GetPolicyDTO(_defaultPolicyName, clientName);
            Assert.NotNull(policyDto);
            Assert.Equal(_applicationName, policyDto.ApplicationName);
            Assert.Equal(_defaultPolicyName, policyDto.PolicyName);
            Assert.Equal(clientName, policyDto.ClientName);
            Assert.Equal(_policyMap.DefaultPolicy.SingleRole, policyDto.SingleRole);
            Assert.Equal(_policyMap.DefaultPolicy.RoleNames, policyDto.Roles);
        }
        [Fact]
        public void can_export_policy()
        {
            var file = Path.GetTempFileName();
            var clientName = "TestClient";
            var policyDto = _policyMap.GetPolicyDTO(_defaultPolicyName, clientName);
            _policyMap.ExportPolicy(policyDto, file);

            var import = JsonConvert.DeserializeObject<PolicyDTO>(File.ReadAllText(file));
            Assert.NotNull(import);
            Assert.Equal(_applicationName, import.ApplicationName);
            Assert.Equal(_defaultPolicyName, import.PolicyName);
            Assert.Equal(clientName, import.ClientName);
            Assert.Equal(_policyMap.DefaultPolicy.SingleRole, import.SingleRole);
            Assert.Equal(_policyMap.DefaultPolicy.RoleNames, import.Roles);
            File.Delete(file);
        }
        [Fact]
        public void will_enforce_single_role() {
            ...
            var user = new UserDetails() {UserId = Guid.NewGuid(), RoleNames = new[] { "role1", "role2" } };
