...
namespace ReactiveDomain.Messaging.Tests
{
        ...
        IHandleCommand<TestTokenCancellableCmd>
    {
        ...
        private long _gotCommand;
        public precanceled_commands_via_cancellation_token()
        {
            _bus = new CommandBus("Test", false, TimeSpan.FromSeconds(2), TimeSpan.FromSeconds(2));

            _bus.Subscribe<TestTokenCancellableCmd>(this);
            var tokenSource = new CancellationTokenSource();

            _bus.Fire(new TestTokenCancellableCmd(false, Guid.NewGuid(), Guid.Empty, tokenSource.Token));
            Assert.True(Interlocked.Read(ref _gotCommand) == 1, "Handler not wired up");
            Interlocked.Exchange(ref _gotCommand, 0);
        ...
        public void canceled_commands_will_not_fire()
        {
            ...
            _bus.Fire(new TestTokenCancellableCmd(false, Guid.NewGuid(), Guid.Empty, tokenSource.Token));
            Thread.Sleep(250);
            Assert.True(Interlocked.Read(ref _gotCommand) == 0, "Handler was triggered");
        }

        public CommandResponse Handle(TestTokenCancellableCmd command) {
            Interlocked.Increment(ref _gotCommand);
