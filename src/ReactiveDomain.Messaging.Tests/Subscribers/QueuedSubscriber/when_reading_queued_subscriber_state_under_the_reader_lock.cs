...
/// <summary>
/// Covers the ReaderLock serialization contract on <see cref="Bus.QueuedSubscriber"/>.
/// </summary>
/// <remarks>
/// The handler is parked mid-mutation on the subscriber's queue thread, so the torn state and the
/// reader's exclusion from it are both facts about the pipeline rather than races. The negative wait
/// can only be beaten by a reader that acquired the lock while the handler still held it.
/// </remarks>
