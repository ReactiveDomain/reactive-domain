...
public class TcpOutboundMessageHandler : IHandle<IMessage>, IDisposable {
	...
	private readonly ConcurrentDictionary<Guid, IMessage> _messagesThatCameFromTcp = new();
	...
	public void IgnoreThisMessage(IMessage message) {
		...
		if (message.MsgId == Guid.Empty) {
			_log.Error(
				$"Message {message.MsgId} (Type {type.Name}) - INTERNAL ERROR: there should NEVER be a message with that MsgId value");
		}
		if (_messagesThatCameFromTcp.TryAdd(message.MsgId, message)) {
			_log.Trace(
				$"Message {message.MsgId} (Type {type.Name}) came from TCP, now added to MessagesThatCameFromTcp hash.  Hash now contains {_messagesThatCameFromTcp.Count} entries.");
		}
	}

	public void Handle(IMessage message) {
		var type = message.GetType();
		if (_messagesThatCameFromTcp.TryRemove(message.MsgId, out _)) {
			_log.Trace(
				$"Message {message.MsgId} (Type {type.Name}) came from TCP originally, NOT sent back on TCP.  MessagesThatCameFromTcp hash now contains {_messagesThatCameFromTcp.Count} entries.");
			return;
		}

		_log.Trace($"Message {message.MsgId} (Type {type.Name}) to be sent over TCP.");
