...
public class JsonMessageSerializer : IEventSerializer {
	...
	public JsonSerializerSettings SerializerSettings {
		get => field ?? StandardSerializerSettings;
		set;
	}
	...
	public JsonMessageSerializer(JsonMessageSerializerSettings? messageSerializerSettings = null, bool preferIMetadataSource = true) {
		messageSerializerSettings ??= new JsonMessageSerializerSettings();
	...
	public EventData Serialize(object @event, IDictionary<string, object>? headers = null) {
		...
		if (!headers.ContainsKey(EventClrQualifiedTypeHeader)) {
			var qualifiedName = FullyQualify ? @event.GetType().AssemblyQualifiedName! : $"{@event.GetType().FullName},{@event.GetType().Assembly.GetName()}";
		...
		if (@event is IMetadataSource source && _preferIMetadataSource) {
			...
			//event name
			commonMetadata.EventName = FullyQualify ? @event.GetType().AssemblyQualifiedName! : @event.GetType().FullName!;
	...
	public object? Deserialize(IEventData @event) {
		...
		if (string.IsNullOrWhiteSpace(clrQualifiedName) || _preferIMetadataSource) {
			...
			if (string.IsNullOrWhiteSpace(clrQualifiedName))
				return null;
		}
	...
	public object? Deserialize(IEventData @event, Type type) {
		var obj = JsonConvert.DeserializeObject(
			@event.Data is not null ? Encoding.UTF8.GetString(@event.Data) : "",
			type,
			SerializerSettings);
		if (obj is IMetadataSource mdObject) {
			try {
				mdObject.Initialize(@event.Metadata is not null
					? new Metadata(JObject.Parse(Encoding.UTF8.GetString(@event.Metadata)),
						JsonSerializer.Create(Json.JsonSettings))
					: new Metadata());
			} catch (Exception) {
				//ignore
			}
	...
	public TEvent? Deserialize<TEvent>(IEventData @event) {
		var evt = JsonConvert.DeserializeObject<TEvent>(
			@event.Data is not null ? Encoding.UTF8.GetString(@event.Data) : "",
			SerializerSettings);
